  ...
  if (!isAdmin) {
    document.getElementById("admin-guard").innerHTML = `
      <div class="empty-state">
        ...
        <div>এই পেজটি শুধু অ্যাডমিনদের জন্য।</div>
      </div>`;
    return;
  }
...
async function loadAdminApps() {
  ...
  if (error || !apps?.length) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:var(--text-faint);padding:30px">এখনো কোনো অ্যাপ যোগ করা হয়নি।</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = apps.map(app => {
    ...
    <tr>
      ...
      <td style="font-family:var(--f-mono);font-size:12px;color:var(--text-faint)">${new Date(app.created_at).toLocaleDateString("bn-BD")}</td>
      <td>
        <button class="btn btn-ghost btn-sm" onclick='editApp(${JSON.stringify(app).replace(/'/g, "&apos;")})'>এডিট</button>
        <button class="btn btn-danger btn-sm" onclick="deleteApp('${app.id}')">ডিলিট</button>
      </td>
    </tr>`;
  }).join("");
}
...
function openModal() {
  IS_EDITING = false;
  document.getElementById("modal-title").textContent = "নতুন অ্যাপ যোগ করুন";
...
function editApp(app) {
  IS_EDITING = true;
  document.getElementById("modal-title").textContent = "অ্যাপ এডিট করুন";
...
async function saveApp(e) {
  ...
  showToast(appId ? "অ্যাপ আপডেট হয়েছে!" : "অ্যাপ যোগ হয়েছে!", "success");
  closeModal();
  loadAdminApps();
}

async function deleteApp(id) {
  if (!confirm("এই অ্যাপটি স্থায়ীভাবে মুছে ফেলতে চান? এর সব রিভিউও মুছে যাবে।")) return;
  ...
  showToast("অ্যাপ মুছে ফেলা হয়েছে।", "success");
  loadAdminApps();
}
